...
HPGe_Coaxial_Properties HPGe_60_TUNL_21033{
    ...
    .hole_face_radius =
        0.5 * 9.3 * mm, // Estimated to be the same as hole radius
    .mount_cup_length = // Calculated from end cap length. Different from (F) in
                        // ORTEC data sheet, because it shows only the part
                        // which contains the detector crystal.
    ...
    .end_cap_to_crystal_gap_side = // Calculated from outer radius and other
                                   // given dimensions
...
HPGe_Coaxial_Properties HPGe_ANL_41203{
    ...
    .mount_cup_length = 8. * inch             // end_cap_length
    ...
    .end_cap_to_crystal_gap_side = 0.5 * 4.25 * inch  // end_cap_outer_radius
