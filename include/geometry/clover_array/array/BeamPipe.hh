 ...
 * read off on the vacuum gauge, which corresponds to about -965(10) hPa or
 * about (1013-965)/1013 = 4.7 % of standard air density/pressure.
 *
 * On the upstream end, the pipe is sealed by a cap.
 * On the downstream end, a pipe with a smaller diameter and an end cap that
 * seals the beam pipe can be slid in.
 */
class BeamPipe {
public:
  BeamPipe(G4LogicalVolume *world_log, const double air_rel_dens = 0.047,
           const bool use_up_cap = true, const bool use_down_pipe = true,
           const bool use_ring = true)
      : world_logical(world_log), air_relative_density(air_rel_dens),
        use_upstream_cap(use_up_cap), use_downstream_pipe(use_down_pipe),
