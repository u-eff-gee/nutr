...
G4VPhysicalVolume *DetectorConstruction::Construct() {
  ...
  G4Tubs *target_solid =
      new G4Tubs("target_solid", 0., 10. * mm, 5. * mm, 0., twopi);
  G4LogicalVolume *target_logical = new G4LogicalVolume(
      target_solid, nist_manager->FindOrBuildMaterial("G4_Al"),
      "target_logical");
  target_logical->SetVisAttributes(G4Color::Red());
  G4VPhysicalVolume *target_physical =
      new G4PVPlacement(new G4RotationMatrix(), G4ThreeVector(), target_logical,
                        "target", world_logical.get(), false, 0);
  source_volumes.push_back(
      make_shared<SourceVolumeTubs>(target_solid, target_physical, 1., 0));
