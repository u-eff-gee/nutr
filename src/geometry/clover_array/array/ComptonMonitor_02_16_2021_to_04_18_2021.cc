...
void ComptonMonitor::Construct(const G4ThreeVector global_coordinates){
    ...
    const double shielding_rotation_angle = 20.*deg;
    const G4ThreeVector shielding_rotation_center(40.*inch*sin(detector_angle), 0., scattering_target_to_target + 40.*inch*cos(detector_angle));
    const double shielding_to_rotation_center = 20.*inch;
    const double shielding_side_y = -0.5*lead_shielding_side_height + 1.8*inch;
    G4Box* lead_shielding_side_solid = new G4Box("lead_shielding_side_solid", 0.5*lead_shielding_side_thickness, 0.5*lead_shielding_side_height, 0.5*lead_shielding_side_length);
    G4LogicalVolume* lead_shielding_side_logical = new G4LogicalVolume(lead_shielding_side_solid, nist->FindOrBuildMaterial("G4_Pb"), "lead_shielding_side_logical");
