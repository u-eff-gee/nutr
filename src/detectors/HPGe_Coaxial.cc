...
void HPGe_Coaxial::Construct(G4LogicalVolume* world_logical, G4ThreeVector global_coordinates)
{
    ...
    G4int nsteps_optimized = opt->Optimize(zPlaneTemp, rInnerTemp, rOuterTemp, zPlane, rInner,
        rOuter, nsteps, detector_name + "_cold_finger_solid");

    G4Polycone *cold_finger_solid = new G4Polycone(detector_name + "_cold_finger_solid", 0. * deg, 360. * deg,
    ...
    nsteps_optimized = opt->Optimize(zPlaneTemp, rInnerTemp, rOuterTemp, zPlane, rInner,
        rOuter, nsteps, detector_name + "_crystal_solid");

    G4Polycone *crystal_solid = new G4Polycone("crystal_solid", 0. * deg, 360. * deg,
        nsteps_optimized, zPlane, rInner, rOuter);
    ...
    auto filter_position_z = Construct_Filters(world_logical, 
        global_coordinates, dist_from_center, theta, phi,
        filter_case.get_filter_case_ring_thickness(), [](std::string name, double radius, double thickness) {
            return new G4Tubs(name, 0., radius,
                thickness * 0.5, 0., twopi);
        });

    if(use_filter_case) {
        filter_case.Construct_Case(global_coordinates, theta, phi,
            dist_from_center - filter_case.get_filter_case_bottom_thickness() * 0.5 - filter_position_z);
    }
    ...
    if(wraps.size()) {
        ...
        for(unsigned int i = 0; i < wraps.size(); ++i) {
            ...
            wrap_solid = new G4Tubs(wrap_solid_name, wrap_radius, wrap_radius + wraps[i].thickness, properties.end_cap_length * 0.5, 0., twopi);
            ...
            wrap_logical = new G4LogicalVolume(wrap_solid,
                nist->FindOrBuildMaterial(wraps[i].material),
                wrap_logical_name);
